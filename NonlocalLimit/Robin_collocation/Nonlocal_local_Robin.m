...
%     g2 = @(x) g(2)/delta;
    ...
    M1 = diag(ones(r+1,1));
    M2 = diag(ones(r+1,1));
    S(1:r+1,1:r+1) = S(1:r+1,1:r+1)+1*al*M1/delta;
    S(end-r:end,end-r:end)= S(end-r:end,end-r:end)-1*al*M2/delta;
    ...
    plot(h*(1:M-1),U(r+1+(1:M-1)'),'LineWidth',1.8,'LineStyle','-','DisplayName',['\delta=' num2str(delta)]); 
    end
    legend
