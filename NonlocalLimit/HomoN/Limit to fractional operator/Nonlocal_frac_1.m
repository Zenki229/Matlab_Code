...
nf = (2:nn-1)';
% Ug = u_sol(vert(nf));
% plot(vert(nf), Ug,'LineWidth',1.8,'LineStyle','-','Marker','o','MarkerIndices',1:4:M,'DisplayName','fractional');
for k = 1:length(del)
    delta = del(k);
    vert_all = -delta:h:1+delta;
    ...
    ff(1:nn_all) = F;
    U = Stiff\ff;
    plot(vert_all,U,'LineWidth',1.8,'LineStyle','-','DisplayName',['\delta=' num2str(delta)]); 
    legend
end